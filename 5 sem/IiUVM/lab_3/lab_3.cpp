...
void PrintData(int channel, int device)
{
    ...
    // Печать занятого объема памяти на диске в МБ.
    cout << endl << setw(30) << setfill(' ') << left << "Занятое место:";
    cout << setprecision(0) << (long double)(((ULONG *)data)[60]) << " МБ";

    // Печать свободного объема памяти на диске в МБ.
    cout << endl << setw(30) << setfill(' ') << left << "Свободное место:";
    cout << setprecision(0) << (long double)(((ULONG *)data)[61]) * 512 / 1024 / 1024 << " МБ";
    ...
    if (HasHDD()) {
        ...
        // Вывод дополнительной информации о HDD или выполнение других операций с ним.
    } else {
